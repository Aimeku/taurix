...
/* ══════════════════════════
   ...
   Supabase envía un código de 6 dígitos
   al email del usuario. No necesita contraseña.
══════════════════════════ */
export async function sendOTP(email) {
  const { error } = await supabase.auth.signInWithOtp({
    email,
    options: { shouldCreateUser: true, emailRedirectTo: "https://taurix.es/" },
  });
  if (error) {
...
export async function verifyOTP(email, token) {
  ...
  if (error) {
    if (error.message.includes("expired")) throw new Error("El código ha expirado. Solicita uno nuevo.");
    if (error.message.includes("invalid") || error.message.includes("Token")) throw new Error("Código incorrecto.");
    throw new Error(error.message);
  }
  return data;
}
...
export async function registerEmail(email, password) {
  const { data, error } = await supabase.auth.signUp({
    email, password, options: { emailRedirectTo: "https://taurix.es/" },
  });
  if (error) {
...
export async function logout() { await supabase.auth.signOut(); window.location.reload(); }
export async function getSession() { const { data: { session } } = await supabase.auth.getSession(); return session; }

/* ══════════════════════════
   MODAL DE AUTENTICACIÓN
   3 tabs: Entrar · Código email · Crear cuenta
══════════════════════════ */
...
export function showResetPasswordModal(recoverySession=null){
  document.getElementById("landingPage")?.classList.add("hidden");document.getElementById("appShell")?.classList.add("hidden");
  const modal=document.createElement("div");modal.id="resetPwModal";
