...
async function _descontarStockSiProcede(checkboxId, docRef = null) {
  ...
  try {
    ...
    const sedeId = document.getElementById("npfSedeId")?.value || null;
    await descontarStockPorVenta(LINEAS, sedeId, docRef, "proforma");
  } catch (e) {
    console.warn("[stock proforma]", e);
  }
...
export async function cargarProformaParaEditar(id){
  ...
  _updateOpUI();
  // Restaurar plantilla_id
  const sel=document.getElementById("npfPlantillaSel");
  if(sel){
    if(p.plantilla_id){
      const opt=sel.querySelector(`option[value="${p.plantilla_id}"]`);
      if(opt){sel.value=p.plantilla_id;}
      else{setTimeout(()=>{const o=sel.querySelector(`option[value="${p.plantilla_id}"]`);if(o)sel.value=p.plantilla_id;},400);}
    } else {sel.value="";}
  }
