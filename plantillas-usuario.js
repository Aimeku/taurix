...
/* ══════════════════════════════════════════════════════
   VARIABLES DE MÓDULO — accesibles por _epTab/_epLang/_epSetAlin
══════════════════════════════════════════════════════ */
let _epLogo   = "";
let _epIdioma = "es";
...
/* _epTab: global, no necesita _epPv */
...
/* _epSetAlin: global, llama _epRunPreview */
...
/* _epLang: global, actualiza idioma y llama preview */
...
/* ══════════════════════════════════════════════════════
   INIT — se llama cuando se navega a la vista
══════════════════════════════════════════════════════ */
window._epInit = function() {
  ...
  // Resetear idioma y logo desde prefill
  ...
  Object.entries(COLS).forEach(function([id, val]) {
  ...
  document.getElementById("ep_logo_btn")?.addEventListener("click", () => logoInp?.click());
  // Limpiar listeners anteriores clonando el input
  if (logoInp) {
    ...
    newInp.addEventListener("change", function(e) {
      ...
      r.onload = function(ev) { _epLogo = ev.target.result; _updateLogoUI(); window._epRunPreview?.(); };
      r.readAsDataURL(f);
    });
  }
  document.getElementById("ep_logo_rm")?.addEventListener("click", function() {
    _epLogo = ""; _updateLogoUI(); window._epRunPreview?.();
  });

  // Toggle mostrar logo estética
  ...
  if (mostLogoLbl && mostLogoChk) {
    const _updateLogoLbl = function() {
      mostLogoLbl.style.borderColor = mostLogoChk.checked ? "var(--accent)" : "var(--brd)";
      mostLogoLbl.style.background  = mostLogoChk.checked ? "var(--accent)0d" : "var(--bg2)";
    };
    _updateLogoLbl();
    mostLogoChk.addEventListener("change", function() { _updateLogoLbl(); window._epRunPreview?.(); });
  }

  // Sync color pickers ↔ hex
  Object.keys(COLS).forEach(function(id) {
    const pick = document.getElementById(id);
    const hex  = document.getElementById(id + "_hex");
    if (!pick || !hex) return;
    pick.addEventListener("input",  function() { hex.value = pick.value; window._epRunPreview?.(); });
    hex.addEventListener("change",  function() { if (/^#[0-9a-f]{6}$/i.test(hex.value)) { pick.value = hex.value; } window._epRunPreview?.(); });
  });

  // (tab/lang/alin se activan abajo, después de definir _epRunPreview)
  ...
  const _lineaHTML = function(l) {
    ...
    return '<div class="linea-row" style="grid-template-columns:1fr 58px 96px 56px 30px">'
      ...
      + '<option value="0"  ' + (l.iva===0 ?"selected":"") + '>0%</option>'
      + '</select>'
      + '<button class="linea-del ep-del-linea" title="Eliminar"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>'
      + '</div>';
  };
  ...
  if (cont) {
    cont.innerHTML = "";
    if (lineas.length) lineas.forEach(function(l) { cont.insertAdjacentHTML("beforeend", _lineaHTML(l)); });
    else cont.insertAdjacentHTML("beforeend", _lineaHTML({}));
  }

  document.getElementById("ep_addLinea")?.addEventListener("click", function() {
    cont?.insertAdjacentHTML("beforeend", _lineaHTML({}));
    window._epRunPreview?.();
  });
  cont?.addEventListener("click", function(e) {
    if (e.target.closest(".ep-del-linea")) { e.target.closest(".linea-row")?.remove(); window._epRunPreview?.(); }
  });
  cont?.addEventListener("input", function() { window._epRunPreview?.(); });

  /* ── FUNCIÓN PREVIEW ── */
  window._epRunPreview = function() {
    const L       = _epIdioma === "en" ? LABELS_EN : LABELS_ES;
    const g       = function(id) { return document.getElementById(id); };
    const gv      = function(id, fb) { const el=g(id); return (el&&el.value) ? el.value : fb; };
    const gc      = function(id, fb) { const el=g(id); return el ? el.checked : fb; };
    ...
    const s = function(id,v) { const el=g(id); if(el) el.textContent=v; };
    s("ep_pv_lbl_de", L.de); s("ep_pv_lbl_para", L.para);

    // Tabla
    const th = g("ep_pv_tabla_head"); if (th) th.style.background = colorAcc;
    s("ep_pv_h_desc",  _epIdioma==="en"?"Description":"Descripción");
    s("ep_pv_h_cant",  L.cant); s("ep_pv_h_precio",L.precio);
    s("ep_pv_h_iva",   L.iva);  s("ep_pv_h_total", L.total);

    const rows = document.querySelectorAll("#ep_lineasContainer .linea-row");
    const larr = [...rows].map(function(r) { return {
    ...
    }; }).filter(function(l) { return l.d || l.p > 0; });
    ...
    if (pvLin) {
      if (!larr.length) {
        ...
        pvLin.innerHTML = larr.map(function(l, ri) {
          const tot = l.c * l.p;
    ...
    const base = larr.reduce(function(a,l){return a+l.c*l.p;},0);
    const iva  = larr.reduce(function(a,l){return a+l.c*l.p*l.i/100;},0);
    const pvTot = g("ep_pv_totales");
    if (pvTot) pvTot.innerHTML = '<table style="margin-left:auto;border-collapse:collapse;font-size:10px;min-width:160px">'
      ...
      + '<tr style="border-top:2px solid ' + colorAcc + '"><td style="font-weight:800;color:#111;padding:4px 12px 2px 0">' + L.totalDoc + '</td><td style="text-align:right;font-family:monospace;font-weight:800;color:' + colorAcc + '">' + (base+iva).toFixed(2) + ' €</td></tr>'
      + '</table>';
  ...
  // Activar tab diseño y resaltar idioma y alineación con los estilos correctos
  window._epTab("diseno");
  window._epLang(_epIdioma);
  window._epSetAlin(prefill.alin_texto || "izq");

  // Conectar todos los inputs al preview (excepto file y hidden)
  document.querySelectorAll("#view-editar-plantilla input:not([type=file]):not([type=hidden]), #view-editar-plantilla select, #view-editar-plantilla textarea").forEach(function(el) {
    if (el.id === "ep_mostrar_logo") return; // ya tiene su listener
    el.addEventListener("input",  function() { window._epRunPreview?.(); });
    el.addEventListener("change", function() { window._epRunPreview?.(); });
  });
  ...
  const btnGuardar  = document.getElementById("epGuardarBtn");
  const btnCancelar = document.getElementById("epCancelarBtn");

  // Limpiar listeners anteriores
  if (btnGuardar) {
    const ng = btnGuardar.cloneNode(true); btnGuardar.parentNode.replaceChild(ng, btnGuardar);
    ng.addEventListener("click", _epGuardar);
  }
  if (btnCancelar) {
    const nc = btnCancelar.cloneNode(true); btnCancelar.parentNode.replaceChild(nc, btnCancelar);
    nc.addEventListener("click", function() { window._switchView?.("plantillas"); });
  }
  ...
  if (isEdit) {
    ...
    if (actions) {
      ...
      delBtn.id = "epEliminarBtn"; delBtn.className = "btn-outline";
  ...
  async function _epGuardar() {
    ...
    const rows = document.querySelectorAll("#ep_lineasContainer .linea-row");
    const lineasArr = [...rows].map(function(r) { return {
    ...
    }; }).filter(function(l) { return l.descripcion || l.precio > 0; });

    const gv = function(id) { return document.getElementById(id)?.value?.trim() || null; };
    const gb = function(id) { return document.getElementById(id)?.checked || false; };
    const gi = function(id, d) { return parseInt(document.getElementById(id)?.value) || d; };
